...
// local notifications.
export async function enableDailyReminders(userId: string): Promise<boolean> {
  ...
  const { error } = await supabase
    .from('push_tokens')
    .upsert({ user_id: userId, token: data, updated_at: new Date().toISOString() }, { onConflict: 'user_id' })
